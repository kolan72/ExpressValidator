...
#pragma warning restore S6966 // Awaitable method should be used
			...
			if (isAsync)
			{
				Assert.ThrowsAsync<InvalidOperationException>(async () => await new ContactValidator().ValidateAsync((Contact)null!));
			}
			else
			{
				Assert.Throws<InvalidOperationException>(() => new ContactValidator().Validate((Contact)null!));
			}
		}
...
#pragma warning restore S6966 // Awaitable method should be used
			...
			if (isAsync)
			{
				Assert.ThrowsAsync<InvalidOperationException>(async () => await new ContactNullableStructValidator().ValidateAsync((ContactStruct?)null));
			}
			else
			{
				Assert.Throws<InvalidOperationException>(() => new ContactNullableStructValidator().Validate((ContactStruct?)null));
			}
		}
